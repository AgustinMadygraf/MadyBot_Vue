...
const checkPhpEndpointHealth = async () => {
  try {
    logger.info('[HttpClientConfig] Verificando salud de PHP_ENDPOINT...');
    await axios.get(`${AppConfig.PHP_ENDPOINT}/health-check`);
    logger.info('[HttpClientConfig] PHP_ENDPOINT está saludable.');
    return true;
  } catch (error) {
    logger.warn('[HttpClientConfig] PHP_ENDPOINT no está saludable:', error.message);
    return false;
  }
};
...
const getApiEndpoint = async () => {
  try {
    logger.info('[HttpClientConfig] Intentando usar API_ENDPOINT de config.json');
    await axios.get(`${AppConfig.API_ENDPOINT}/health-check`);
    logger.info('[HttpClientConfig] API_ENDPOINT de config.json funcionando.');
    return AppConfig.API_ENDPOINT;
  } catch (error) {
    logger.warn('[HttpClientConfig] Fallback: Obteniendo API_ENDPOINT desde PHP...');
    try {
      ...
      if (!response.data.endpoint) {
        logger.warn('[HttpClientConfig] El campo "endpoint" está undefined en la respuesta de PHP. Usando valor por defecto.');
        throw new Error('El endpoint devuelto por PHP es undefined');
      }
      logger.info('[HttpClientConfig] API_ENDPOINT obtenido desde PHP:', response.data.endpoint);
      return response.data.endpoint;
    } catch (phpError) {
      logger.error('[HttpClientConfig] Error al obtener API_ENDPOINT desde PHP:', phpError.message);
